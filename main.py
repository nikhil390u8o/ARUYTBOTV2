...
BOT_TOKEN  = os.environ.get("BOT_TOKEN",      "7982215226:AAEU4ok_OLeJvutUjY6b2M4YGfzIC83zApQ")
...
def auth_kb():
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("🔐 ʟᴏɢɪɴ", callback_data="do_login", style=ButtonStyle.PRIMARY),
        InlineKeyboardButton("📝 ʀᴇɢɪsᴛᴇʀ", callback_data="do_register", style=ButtonStyle.SUCCESS),
    ]])

def main_kb():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🔑 ᴍʏ ᴋᴇʏs", callback_data="my_keys", style=ButtonStyle.PRIMARY),
            InlineKeyboardButton("📈 ᴜsᴀɢᴇ", callback_data="my_usage", style=ButtonStyle.PRIMARY),
        ],
        [
            InlineKeyboardButton("➕ ɴᴇᴡ ᴋᴇʏ", callback_data="gen_key", style=ButtonStyle.SUCCESS),
            InlineKeyboardButton("🏓 ᴘɪɴɢ", callback_data="ping", style=ButtonStyle.PRIMARY),
        ],
        [
            InlineKeyboardButton("💳 ᴜᴘɢʀᴀᴅᴇ", callback_data="plans", style=ButtonStyle.SUCCESS),
            InlineKeyboardButton("📜 ʜɪsᴛᴏʀʏ", callback_data="pay_history", style=ButtonStyle.PRIMARY),
        ],
        [
            InlineKeyboardButton("🌐 ᴡᴇʙ ᴅᴀsʜʙᴏᴀʀᴅ", web_app=WebAppInfo(url=f"{API_BASE}")),
        ],
        [
            InlineKeyboardButton("🚪 ʟᴏɢᴏᴜᴛ", callback_data="logout", style=ButtonStyle.DANGER),
        ]
    ])

def back_kb(cb="main_menu"):
    return InlineKeyboardMarkup([[InlineKeyboardButton("🏠 ʙᴀᴄᴋ", callback_data=cb, style=ButtonStyle.PRIMARY)]])
...
# ══════════════════════════════════════════════════════════════════
#  ✅ CORE HELPER — channel join gate for ALL callbacks
#  Returns True if joined, False + sends alert if not
# ══════════════════════════════════════════════════════════════════
async def gate(client, cb: CallbackQuery) -> bool:
    """
    Call at the top of every callback handler.
    If user has left the channel:
      - show popup alert
      - edit caption back to join screen
      - return False  →  handler should return immediately
    """
    if not await check_joined(client, cb.from_user.id):
        ...
        # Edit caption back to join screen (ignore errors if caption unchanged)
        try:
...
async def check_join_cb(client, cb: CallbackQuery):
    ...
    if sess:
        await cb.message.edit_caption(
            main_caption(cb.from_user, sess["username"]),
            reply_markup=main_kb())
    else:
        await cb.message.edit_caption(
            f"**⚡ ᴀʀᴜ ʏᴛ ᴀᴘɪ ʙᴏᴛ**\n\n🔐 ʟᴏɢɪɴ ᴏʀ 📝 ʀᴇɢɪsᴛᴇʀ",
...
async def text_handler(client, message: Message):
    ...
    # Join check for text input flow too
    if not await check_joined(client, uid):
        _pending.pop(uid, None)
        _reg_data.pop(uid, None)
        await message.reply(
            "**❌ ᴄʜᴀɴɴᴇʟ ʟᴇꜰᴛ ᴅᴇᴛᴇᴄᴛᴇᴅ!**\n\n"
            "📢 Pehle channel join karo → /start",
