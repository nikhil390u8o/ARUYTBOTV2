...
BOT_TOKEN  = os.environ.get("BOT_TOKEN",      "8808019830:AAEE05lxb1tIVLPWsMIfsLKYuR8NBPqwB6I")
    ...
    if not keys:
        await cb.message.edit_caption(
            ...
            reply_markup=InlineKeyboardMarkup([[
                btn("➕ ɴᴇᴡ ᴋᴇʏ", callback_data="gen_key", style=_SUCCESS),
                btn("🏠 ʙᴀᴄᴋ", callback_data="main_menu", style=_PRIMARY),
            ]])); return
    text = "**🔑 ʏᴏᴜʀ ᴀᴘɪ ᴋᴇʏs**\n\n"
    for k in keys:
...
async def my_usage_cb(client, cb: CallbackQuery):
    ...
    await cb.message.edit_caption(
        ...
        f"╚══════════════════╝",
        reply_markup=InlineKeyboardMarkup([[
            btn("🔄 ʀᴇꜰʀᴇsʜ", callback_data="my_usage", style=_PRIMARY),
            btn("🏠 ʙᴀᴄᴋ", callback_data="main_menu", style=_PRIMARY),
        ]]))
...
async def plans_cb(client, cb: CallbackQuery):
    ...
    await cb.message.edit_caption(text,
        reply_markup=InlineKeyboardMarkup([[
            btn("🌐 ᴏᴘᴇɴ ᴅᴀsʜʙᴏᴀʀᴅ", web_app=WebAppInfo(url=f"{API_BASE}"), style=_SUCCESS),
        ],[
            btn("🏠 ʙᴀᴄᴋ", callback_data="main_menu", style=_PRIMARY),
        ]]))
...
async def ping_cb(client, cb: CallbackQuery):
    ...
    await cb.message.edit_caption(
        ...
        f"╚══════════════════╝",
        reply_markup=InlineKeyboardMarkup([[
            btn("🔄 ʀᴇꜰʀᴇsʜ", callback_data="ping", style=_PRIMARY),
            btn("🏠 ʙᴀᴄᴋ", callback_data="main_menu", style=_PRIMARY),
        ]]))
